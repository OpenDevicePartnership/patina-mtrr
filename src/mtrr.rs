...
impl<H: Hal> MtrrLib<H> {
    ...
    fn mtrr_lib_set_memory_type(
        ...
        // Reserve space for the new ranges. copy_within is memmove-based, so it's safe for
        // overlapping src/dest in either direction (array growing or shrinking).
        let dest = (end_index as i64 + 1 - delta_count) as usize;
        working_ranges.copy_within((end_index + 1)..*working_ranges_count, dest);
